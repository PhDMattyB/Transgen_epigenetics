...
vif.cca(func_ORIG_clean_RDA_full)
...
# functional orig opercular kt --------------------------------------------

func_ORIG_operc_RDA_full = rda(func_clean_mvalues_only ~ Opercular_KT + ecotype + Opercular_KT*ecotype + csize_real, 
                               data = func_clean_pheno_fish_final, 
                               scale = T)
RsquareAdj(func_ORIG_operc_RDA_full)
summary(eigenvals(func_ORIG_operc_RDA_full, 
                  model = 'constrained'))
screeplot(func_ORIG_operc_RDA_full)
func_ORIG_opercular_signif_full = anova.cca(func_ORIG_operc_RDA_full, 
                                  parallel = getOption('mc.cores'))
...
func_WGP_premax_RDA_full = rda(func_clean_mvalues_only ~ PreMax_KT_F2 + ecotype + PreMax_KT_F2*ecotype + csize_real, 
                              data = func_clean_pheno_fish_final, 
                              scale = T)
...
func_WGP_premax_sum = summary(func_WGP_premax_RDA_full)
...
summary(eigenvals(func_WGP_operc_RDA_full, 
                  model = 'constrained'))
screeplot(func_WGP_operc_RDA_full)
func_WGP_operc_signif_full = anova.cca(func_WGP_operc_RDA_full, 
                                        parallel = getOption('mc.cores'))


vif.cca(func_WGP_operc_RDA_full)

func_WGP_operc_sum = summary(func_WGP_operc_RDA_full)

func_WGP_operc_sum$species %>%
  as_tibble() %>%
  write_csv('func_WGP_operc_RDA_Uncorrected_PCA_locations.csv')

func_WGP_operc_sum$sites %>%
  as_tibble() %>%
  write_csv('func_WGP_operc_RDA_Uncorrected_PCA_individuals.csv')

func_WGP_operc_sum$biplot %>%
  as_tibble() %>%
  write_csv('func_WGP_operc_RDA_Uncorrected_PCA_biplot.csv')


func_WGP_operc_rda_scores = scores(func_WGP_operc_RDA_full,
                                    choices = c(1:5),
                                    display = 'species')
# 
hist(func_WGP_operc_rda_scores[,1])
# hist(func_rda_scores[,2])
# hist(func_rda_scores[,3])
# hist(func_rda_scores[,4])
# hist(func_rda_scores[,5])
# 
func_WGP_operc_rda_outliers_axis1 = outliers(func_WGP_operc_rda_scores[,1], 3)
# 
# 
func_WGP_operc_rda_outliers_axis1 = cbind.data.frame(rep(1,
                                                          times = length(func_WGP_operc_rda_outliers_axis1)),
                                                      names(func_WGP_operc_rda_outliers_axis1),
                                                      unname(func_WGP_operc_rda_outliers_axis1))
# 
func_WGP_operc_rda_outliers_axis1 = func_WGP_operc_rda_outliers_axis1 %>%
  as_tibble() %>%
  dplyr::rename(axis = 1,
                loc = 2,
                scores = 3)
# 
# 
func_WGP_operc_all_loc = func_WGP_operc_rda_scores[,1]
# 
func_WGP_operc_all_loc = cbind.data.frame(rep(2,
                                               times = length(func_WGP_operc_all_loc)),
                                           names(func_WGP_operc_all_loc),
                                           unname(func_WGP_operc_all_loc))
func_WGP_operc_all_loc = func_WGP_operc_all_loc %>%
  as_tibble() %>%
  dplyr::rename(axis = 1,
                loc = 2,
                scores = 3)
# 
func_WGP_operc_all_loc = func_WGP_operc_all_loc[!func_WGP_operc_all_loc$loc %in% func_WGP_operc_rda_outliers_axis1$loc,]
# 
write_csv(func_WGP_operc_all_loc,
          'func_WGP_operc_RDA_PCaxes_nonoutliers_methylation.csv')

write_csv(func_WGP_operc_rda_outliers_axis1,
          'func_WGP_operc_RDA_outliers_AXIS1_RAW_PCaxes_methylation.csv')
# 
func_WGP_operc_rda_out = as.data.frame(func_WGP_operc_rda_outliers_axis1)
func_WGP_operc_all_loc = as.data.frame(func_WGP_operc_all_loc)
func_pheno_fish_final = as.data.frame(func_clean_pheno_fish_final)
# 
func_WGP_phenotypes = func_clean_pheno_fish_final %>%
  as_tibble() %>%
...
#                   'eco_num', 
#                   'interaction')
colnames(foo) = c('WGP_operc_kt',
                  'eco_num', 
                  'Interaction', 
                  'csize')

func_WGP_operckt = func_WGP_phenotypes %>% 
  dplyr::select(Opercular_KT_F2, 
                eco_num, 
                csize_real) %>%
  mutate(interaction = Opercular_KT_F2*eco_num)

for (i in 1:length(func_WGP_operc_rda_out$loc)){
  nam = func_WGP_operc_rda_out[i,2]
  loc.gen = func_clean_mvalues_only[,nam]
  foo[i,] = apply(func_WGP_operckt,2,function(x)cor(x,loc.gen))
}

func_WGP_operc_candidates = cbind.data.frame(func_WGP_operc_rda_out,
                                              foo)

func_WGP_operc_candidates %>%
  as_tibble() %>%
  write_csv('func_WGP_operc_RDA_outliers_methylation_correlations.csv')
# 
##check for duplicates
length(func_WGP_operc_candidates$loc[duplicated(func_WGP_operc_candidates$loc)])
# 
for(i in 1:length(func_WGP_operc_candidates$loc)){
  bar = func_WGP_operc_candidates[i,]
  func_WGP_operc_candidates[i,8] = names(which.max(abs(bar[4:7])))
  func_WGP_operc_candidates[i,9] = max(abs(bar[4:7]))
}
# 
func_WGP_operc_candidates %>% 
  as_tibble() %>%
  rename(Association = V8, 
         Correlation = V9) %>%
  write_csv("func_WGP_operc_RDA_CAND_corr.csv") %>% 
  group_by(Association) %>% 
  summarize(n = n())
...
Body_clean_data = read_csv('BODY_tgp_wgp_CandiscCleaned_data.csv') %>% 
  inner_join(.,
             meta_data, 
             by = 'fish', 
             relationship = 'many-to-many')
...
BODY_pheno_fish = Body_clean_data %>% 
  filter(str_detect(fish,
                    '_#G')) %>% 
  distinct(fish, 
           .keep_all = T) %>% 
...
BODY_pheno_fish_ID = BODY_pheno_fish %>% 
  separate_wider_regex(fish, 
                       c(var1 = ".*?", 
                         "_", 
                         var2 = ".*")) %>% 
...
BODY_pheno_fish_ID$var1 = gsub("'", '', BODY_pheno_fish_ID$var1)

BODY_pheno_fish_ID = BODY_pheno_fish_ID %>% 
  unite(col = 'Fish_ID', 
        sep = '')%>% 
  mutate(Fish_ID = gsub("Myvat", 
                        "MYV", 
...
BODY_clean_pheno_fish_final = inner_join(meth_fish_ID, 
                                        BODY_pheno_fish) %>% 
  arrange(Fish_ID)
...
BODY_clean_mvalues_final = bind_cols(meth_fish_ID, 
                                    mvalues) %>% 
  arrange(Fish_ID)  %>% 
  filter(Fish_ID != 'GTS1812_EU1_#G1', 
         Fish_ID != 'GTS1812_EU1_#G2',
...
BODY_clean_mvalues_final$Fish_ID == BODY_clean_pheno_fish_final$Fish_ID
...
#         TGP_clean_pheno_fish_final$Fish_ID)
## TGP RDA 
BODY_clean_mvalues_only = BODY_clean_mvalues_final %>% 
  # select(-1) %>% 
  select(-Location_data, 
         -Fish_ID)

BODY_TGP_RDA_full = rda(BODY_clean_mvalues_only ~ TGPclean + ecotype + TGPclean*ecotype + csize_real, 
                         data = BODY_clean_pheno_fish_final, 
                         scale = T)
...
RsquareAdj(BODY_TGP_RDA_full)
summary(eigenvals(BODY_TGP_RDA_full, 
                  model = 'constrained'))

screeplot(BODY_TGP_RDA_full)

## Run after all other coding is finished
## This will take a while 
BODY_TGP_signif_full = anova.cca(BODY_TGP_RDA_full, 
                            parallel = getOption('mc.cores'))

# TGP_eco_signif_full2 = anova.cca(TGP_clean_eco_RDA2,
#                                  parallel = getOption('mc.cores'))

vif.cca(BODY_TGP_RDA_full)

BODY_TGP_clean_sum = summary(BODY_BODY_TGP_RDA_full)

BODY_TGP_clean_sum$species %>%
  as_tibble() %>%
  write_csv('BODY_TGP_clean_RDA_Uncorrected_PCA_locations.csv')

BODY_TGP_clean_sum$sites %>%
  as_tibble() %>%
  write_csv('BODY_TGP_clean_RDA_Uncorrected_PCA_individuals.csv')

BODY_TGP_clean_sum$biplot %>%
  as_tibble() %>%
  write_csv('BODY_TGP_clean_RDA_Uncorrected_PCA_biplot.csv')


BODY_TGP_clean_rda_scores = scores(BODY_TGP_RDA_full,
                              choices = c(1:5),
                              display = 'species')
# 
hist(BODY_TGP_clean_rda_scores[,1])
# hist(BODY_TGP_rda_scores[,2])
# hist(BODY_TGP_rda_scores[,3])
# hist(BODY_TGP_rda_scores[,4])
# hist(BODY_TGP_rda_scores[,5])
# 
BODY_TGP_clean_rda_outliers_axis1 = outliers(BODY_TGP_clean_rda_scores[,1], 3)
# 
# 
BODY_TGP_clean_rda_out_axis1 = cbind.data.frame(rep(1,
                                               times = length(BODY_TGP_clean_rda_outliers_axis1)),
                                           names(BODY_TGP_clean_rda_outliers_axis1),
                                           unname(BODY_TGP_clean_rda_outliers_axis1))
# 
BODY_TGP_clean_rda_out_axis1 = BODY_TGP_clean_rda_out_axis1 %>%
  as_tibble() %>%
  dplyr::rename(axis = 1,
                loc = 2,
                scores = 3)
# 
# 
BODY_TGP_all_loc = BODY_TGP_clean_rda_scores[,1]
# 
BODY_TGP_rda_normal = cbind.data.frame(rep(2,
                                      times = length(BODY_TGP_all_loc)),
                                  names(BODY_TGP_all_loc),
                                  unname(BODY_TGP_all_loc))
BODY_TGP_rda_normal = BODY_TGP_rda_normal %>%
  as_tibble() %>%
  dplyr::rename(axis = 1,
                loc = 2,
                scores = 3)
# 
BODY_TGP_rda_normal = BODY_TGP_rda_normal[!BODY_TGP_rda_normal$loc %in% BODY_TGP_clean_rda_out_axis1$loc,]
# 
write_csv(BODY_TGP_rda_normal,
          'BODY_TGP_clean_RDA_PCaxes_nonoutliers_methylation.csv')

write_csv(BODY_TGP_clean_rda_out_axis1,
          'BODY_TGP_clean_RDA_outliers_AXIS1_RAW_PCaxes_methylation.csv')
# 
BODY_TGP_clean_rda_out = as.data.frame(BODY_TGP_clean_rda_out_axis1)
BODY_TGP_all_loc = as.data.frame(BODY_TGP_all_loc)
BODY_TGP_clean_pheno_fish_final = as.data.frame(BODY_clean_pheno_fish_final)
# BODY_clean_pheno_fish_final
BODY_TGP_clean_phenotypes = BODY_TGP_clean_pheno_fish_final %>%
  as_tibble() %>%
  mutate(eco_num = as.numeric(case_when(
    ecotype == 'c' ~ '1',
    ecotype == 'w' ~ '2'))) %>%
...
BODY_TGP_clean_phenotypes = BODY_TGP_clean_phenotypes %>% 
  dplyr::select(TGPclean, 
                eco_num, 
                csize_real) %>%
  mutate(interaction = TGPclean*eco_num)

for (i in 1:length(BODY_TGP_clean_rda_out$loc)){
  nam = BODY_TGP_clean_rda_out[i,2]
  loc.gen = BODY_clean_mvalues_only[,nam]
  foo[i,] = apply(BODY_TGP_clean_phenotypes,2,function(x)cor(x,loc.gen))
}

BODY_TGP_clean_candidates = cbind.data.frame(BODY_TGP_clean_rda_out,
                                        foo)

BODY_TGP_clean_candidates %>%
  as_tibble() %>%
  write_csv('BODY_TGP_clean_RDA_outliers_methylation_correlations.csv')
# 
##check for duplicates
length(BODY_TGP_clean_candidates$loc[duplicated(BODY_TGP_clean_candidates$loc)])
# 
for(i in 1:length(BODY_TGP_clean_candidates$loc)){
  bar = BODY_TGP_clean_candidates[i,]
  BODY_TGP_clean_candidates[i,8] = names(which.max(abs(bar[4:7])))
  BODY_TGP_clean_candidates[i,9] = max(abs(bar[4:7]))
}
# 
BODY_TGP_clean_candidates %>% 
  as_tibble() %>%
  rename(Association = V8, 
         Correlation = V9) %>%
  write_csv("BODY_TGP_clean_RDA_CAND_corr.csv") %>% 
  group_by(Association) %>% 
  summarize(n = n())
...
BODY_WGP_RDA_full = rda(BODY_clean_mvalues_only ~ WGPclean + ecotype + WGPclean*ecotype + csize_real, 
                        data = BODY_clean_pheno_fish_final, 
                        scale = T)
...
RsquareAdj(BODY_WGP_RDA_full)
summary(eigenvals(BODY_WGP_RDA_full, 
                  model = 'constrained'))

screeplot(BODY_WGP_RDA_full)

## Run after all other coding is finished
## This will take a while 
BODY_WGP_signif_full = anova.cca(BODY_WGP_RDA_full, 
                                 parallel = getOption('mc.cores'))
...
BODY_WGP_clean_sum$species %>%
  as_tibble() %>%
  write_csv('BODY_WGP_clean_RDA_Uncorrected_PCA_locations.csv')

BODY_WGP_clean_sum$sites %>%
  as_tibble() %>%
  write_csv('BODY_WGP_clean_RDA_Uncorrected_PCA_individuals.csv')

BODY_WGP_clean_sum$biplot %>%
  as_tibble() %>%
  write_csv('BODY_WGP_clean_RDA_Uncorrected_PCA_biplot.csv')


BODY_WGP_clean_rda_scores = scores(BODY_WGP_RDA_full,
                                   choices = c(1:5),
                                   display = 'species')
# 
hist(BODY_WGP_clean_rda_scores[,1])
# hist(BODY_WGP_rda_scores[,2])
# hist(BODY_WGP_rda_scores[,3])
# hist(BODY_WGP_rda_scores[,4])
# hist(BODY_WGP_rda_scores[,5])
# 
BODY_WGP_clean_rda_outliers_axis1 = outliers(BODY_WGP_clean_rda_scores[,1], 3)
# 
# 
BODY_WGP_clean_rda_out_axis1 = cbind.data.frame(rep(1,
                                                    times = length(BODY_WGP_clean_rda_outliers_axis1)),
                                                names(BODY_WGP_clean_rda_outliers_axis1),
                                                unname(BODY_WGP_clean_rda_outliers_axis1))
# 
BODY_WGP_clean_rda_out_axis1 = BODY_WGP_clean_rda_out_axis1 %>%
  as_tibble() %>%
  dplyr::rename(axis = 1,
                loc = 2,
                scores = 3)
# 
# 
BODY_WGP_all_loc = BODY_WGP_clean_rda_scores[,1]
# 
BODY_WGP_rda_normal = cbind.data.frame(rep(2,
                                           times = length(BODY_WGP_all_loc)),
                                       names(BODY_WGP_all_loc),
                                       unname(BODY_WGP_all_loc))
BODY_WGP_rda_normal = BODY_WGP_rda_normal %>%
  as_tibble() %>%
  dplyr::rename(axis = 1,
                loc = 2,
                scores = 3)
# 
BODY_WGP_rda_normal = BODY_WGP_rda_normal[!BODY_WGP_rda_normal$loc %in% BODY_WGP_clean_rda_out_axis1$loc,]
# 
write_csv(BODY_WGP_rda_normal,
          'BODY_WGP_clean_RDA_PCaxes_nonoutliers_methylation.csv')

write_csv(BODY_WGP_clean_rda_out_axis1,
          'BODY_WGP_clean_RDA_outliers_AXIS1_RAW_PCaxes_methylation.csv')
# 
BODY_WGP_clean_rda_out = as.data.frame(BODY_WGP_clean_rda_out_axis1)
BODY_WGP_all_loc = as.data.frame(BODY_WGP_all_loc)
BODY_WGP_clean_pheno_fish_final = as.data.frame(BODY_clean_pheno_fish_final)
# BODY_clean_pheno_fish_final
BODY_WGP_clean_phenotypes = BODY_WGP_clean_pheno_fish_final %>%
  as_tibble() %>%
  mutate(eco_num = as.numeric(case_when(
    ecotype == 'c' ~ '1',
    ecotype == 'w' ~ '2'))) %>%
...
BODY_WGP_clean_phenotypes = BODY_WGP_clean_phenotypes %>% 
  dplyr::select(WGPclean, 
                eco_num, 
                csize_real) %>%
  mutate(interaction = WGPclean*eco_num)

for (i in 1:length(BODY_WGP_clean_rda_out$loc)){
  nam = BODY_WGP_clean_rda_out[i,2]
  loc.gen = BODY_clean_mvalues_only[,nam]
  foo[i,] = apply(BODY_WGP_clean_phenotypes,2,function(x)cor(x,loc.gen))
}

BODY_WGP_clean_candidates = cbind.data.frame(BODY_WGP_clean_rda_out,
                                             foo)

BODY_WGP_clean_candidates %>%
  as_tibble() %>%
  write_csv('BODY_WGP_clean_RDA_outliers_methylation_correlations.csv')
# 
##check for duplicates
length(BODY_WGP_clean_candidates$loc[duplicated(BODY_WGP_clean_candidates$loc)])
# 
for(i in 1:length(BODY_WGP_clean_candidates$loc)){
  bar = BODY_WGP_clean_candidates[i,]
  BODY_WGP_clean_candidates[i,8] = names(which.max(abs(bar[4:7])))
  BODY_WGP_clean_candidates[i,9] = max(abs(bar[4:7]))
}
# 
BODY_WGP_clean_candidates %>% 
  as_tibble() %>%
  rename(Association = V8, 
         Correlation = V9) %>%
  write_csv("BODY_WGP_clean_RDA_CAND_corr.csv") %>% 
  group_by(Association) %>% 
  summarize(n = n())
